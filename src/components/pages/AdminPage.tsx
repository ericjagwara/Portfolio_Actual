        ...
        {tab==="blogs"&&!loading&&<div className="space-y-2">{blogs.map(b=>(
          <div key={b.id} className="flex items-center gap-3 px-4 py-3 rounded-xl" style={{background:C.card,border:`1px solid ${C.border}`}}>
            ...
            <div className="flex gap-1.5 flex-shrink-0">
              <button onClick={()=>togBlog(b)} className="p-1.5 rounded-lg hover:opacity-80" style={{background:C.border}}>{b.published?<Eye size={14} style={{color:C.teal}}/>:<EyeOff size={14} style={{color:C.muted}}/>}</button>
