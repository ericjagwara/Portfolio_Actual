...
export function BlogPostPage({ setPage }: BlogPostPageProps) {
  ...
  useEffect(() => {
    ...
    fetchBlogPost(slug)
      .then((p) => {
        setPost(p);
        setLoading(false);
        return fetchRelatedPosts(slug, p.tags, 4);
      })
      .then((r) => setRelated(r || []))
  ...
  if (loading) {
    return <section className="min-h-screen py-24 px-6"><div className="max-w-3xl mx-auto text-center" style={{ color: C.muted }}>Loading...</div></section>;
  }
    ...
    return (
      <section className="min-h-screen py-24 px-6">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <button onClick={() => { setPage("Blog"); nav("/blog"); }} className="text-sm hover:underline" style={{ color: C.accent }}>← Back to Blog</button>
        </div>
      </section>
    );
  }
  ...
  const schema = {
    ...
    "author": { "@type": "Person", "@id": `${BASE}/#person`, "name": "Eric Jagwara", "url": BASE },
    "publisher": { "@type": "Person", "@id": `${BASE}/#person` },
    "mainEntityOfPage": { "@type": "WebPage", "@id": url }, "url": url,
    "image": `${BASE}/images/blog-og.png`, "keywords": post.tags.join(", "),
    "inLanguage": pl === "pt" ? "pt-AO" : pl === "fr" ? "fr-CD" : "en",
    "wordCount": wc, "articleSection": post.tags[0] || "Technology",
  ...
  const renderInline = (text: string, kp = ""): React.ReactNode[] => {
    ...
    while (rem.length > 0) {
      ...
      const bl = rem.match(/\*\*([^*]+)\*\*/);
      const ms = [
        ml ? { t: "l" as const, m: ml, i: rem.indexOf(ml[0]) } : null,
        bl ? { t: "b" as const, m: bl, i: rem.indexOf(bl[0]) } : null,
      ].filter(Boolean).sort((a, b) => a!.i - b!.i);
      if (!ms.length) { els.push(rem); break; }
      const e = ms[0]!;
      if (e.i > 0) els.push(rem.substring(0, e.i));
      if (e.t === "l") {
        const [, lt, lu] = e.m!;
        const isInt = lu.startsWith("/");
        els.push(
          isInt
            ? <Link key={`${kp}-${ki++}`} to={lu} className="underline hover:opacity-80" style={{ color: C.accent }}>{lt}</Link>
            : <a key={`${kp}-${ki++}`} href={lu} target="_blank" rel="noopener noreferrer" className="underline hover:opacity-80" style={{ color: C.accent }}>{lt}</a>
        );
      } else {
        els.push(<strong key={`${kp}-${ki++}`} style={{ color: C.bright }}>{e.m![1]}</strong>);
      }
  ...
  return (
    <section className="min-h-screen py-24 px-6">
      <Helmet>
        ...
        <meta property="og:image" content={`${BASE}/images/blog-og.png`} />
        ...
        <meta name="twitter:image" content={`${BASE}/images/blog-og.png`} />
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <FadeIn delay={0.05}>
          <header className="mb-8">
            <div className="flex flex-wrap gap-2 mb-4">
              <span className="text-xs px-3 py-1 rounded-full flex items-center gap-1" style={{ background: C.accent + "20", color: C.accent }}>{LF[pl]} {LN[pl]}</span>
              {post.tags.filter((t) => !["Português", "Français", "English"].includes(t)).map((tag) => <span key={tag} className="text-xs px-3 py-1 rounded-full" style={{ background: C.border, color: C.text }}>{tag}</span>)}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs" style={{ color: C.muted, fontFamily: "'Space Mono',monospace" }}>
              ...
              <span className="flex items-center gap-1.5"><BookOpen size={14} />{wc.toLocaleString()} words</span>
            </div>
          </header>
        </FadeIn>
        ...
        <FadeIn delay={0.18}>
          <div className="mt-8 pt-6" style={{ borderTop: `1px solid ${C.border}` }}>
            <div className="flex flex-wrap gap-2 items-center">
              ...
              {post.tags.map((tag) => <span key={tag} className="text-xs px-2 py-1 rounded" style={{ background: C.border, color: C.text }}>{tag}</span>)}
            </div>
          </div>
        </FadeIn>
        ...
        {related.length > 0 && (
          <FadeIn delay={0.2}>
            <div className="mt-10">
              <h3 className="text-lg font-bold mb-4" style={{ color: C.bright, fontFamily: "'Inter',sans-serif" }}>Related Articles</h3>
              <div className="grid gap-4 sm:grid-cols-2">
                {related.map((r) => {
                  const rl = r.language || "en";
                  return (
                    <Link key={r.slug} to={`/blog/${r.slug}`} className="block">
                      <Cd className="h-full hover:border-opacity-50 transition-all" style={{ borderColor: C.borderLight }}>
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-sm">{LF[rl]}</span>
                          <span className="text-xs" style={{ color: C.muted, fontFamily: "'Space Mono',monospace" }}>{new Date(r.date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</span>
                        </div>
        ...
        <FadeIn delay={0.25}>
          <Cd className="mt-10">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h4 className="font-bold text-base mb-1" style={{ color: C.bright }}>Eric Jagwara</h4>
                <p className="text-xs mb-3" style={{ color: C.muted }}>AI & Embedded Systems Engineer · Zindi Uganda Ambassador · Young AI Leaders Kampala Hub Leader</p>
                <div className="flex gap-3"><EL href="https://twitter.com/ericjagwara">Twitter</EL><EL href="https://linkedin.com/in/ericjagwara">LinkedIn</EL><EL href="https://github.com/ericjagwara">GitHub</EL></div>
              </div>
            </div>
          </Cd>
        </FadeIn>
